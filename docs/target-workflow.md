...
同一 computation key 的 worker 重试会先复用输入版本完全一致的已完成通道，只重新调用
失败/超时通道；后续上游选择产生新 job 与新 generation，不复用已失效的下游结果。目录外
概念的审核映射仅影响**之后**落库的候选解析和资产队列，历史 exposure 快照不回写。映射目前
是受控的 operations service，尚未提供匿名客户端可调用的管理端点。
